...
class ObjectChunkingPlugin extends ServerPlugin {
	...
	private const OBJECT_UPLOAD_HEADER = 'X-Nextcloud-Object-Chunking';
	private const OBJECT_UPLOAD_DESTINATION_HEADER = 'X-Nextcloud-Object-Destination';
	 ...
	 */
	private function getTargetFile(string $path, $createIfNotExists = false) {
		try {
	...
	public function beforeMkcol(RequestInterface $request, ResponseInterface $response): bool {
		...
		$targetPath = $this->server->httpRequest->getHeader(self::OBJECT_UPLOAD_DESTINATION_HEADER);
		if (!$targetPath) {
	...
	public function beforePut(RequestInterface $request, ResponseInterface $response): bool {
		...
		$storage->putChunkedFilePart($targetFile->getInternalPath(), $uploadId, (string)$partId, $stream, $request->getHeader('Content-Length'));
